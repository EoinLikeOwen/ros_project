...
main (int argc, char* argv[])
{
	...
	//The largest scale to use in thr DoN filter.
	double scale2;
	...
	/**
	* NOTE: setting viewpoint is very impotant, so that we can ensure
	...
	*/
	...
	//calculate normals iwth the large scale
	cout << "Calcuating normals for scale.." <<scale2 <<endl;
	...
	if (!don.initCompute ())
	{
		std::cerr << "Error: Could intialize DoN feature operator" << std::endl;
		exit (EXIT_FAILURE);
	}
	...
	for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end(); ++it, j++)
	{
		...
		//Save Cluster
		cout << "PointCloud repersenting the Cluster: "<< cloud_cluster_don->points.size() <<  "data points."<< std::endl;
		stringstream ss;
